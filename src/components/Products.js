...
const Products = () => {
  ...
  const handleQuantity = (productId,qty) =>{
    const { balance, token } = localStorage;
  ...
  const checkItems = (event) => {
    ...
    if (!username) {
      ...
      if (isItemInCart(cart, event.target.value)) {
        enqueueSnackbar(
        ...
        const { balance, token } = localStorage;
        addToCart(token, cart, products, event.target.value, 1, {
          preventDuplicate: true,
        });
      }
    }
      ...
      <Grid container>
          ...
          <Box padding={2}>
            ...
            {!loader && (
              <Grid container spacing={2} mt={2}>
                {products.map((x) => {
                  return (
                    <Grid item md={3} xs={6} key={x._id}>
                      <ProductCard product={x} handleAddToCart={checkItems} />
                    </Grid>
                  );
                })}
              </Grid>
        ...
        {localStorage.getItem("username") && (
          <Grid item md={3} style={{ backgroundColor: "#E9F5E1" }}>
            <Cart
              products={products}
              items={generateCartItemsFrom(cart, products)}
